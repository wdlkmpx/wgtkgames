...
gint _user_action_event(GtkWidget *w, GdkEvent *ev)
{
    ...
    if (ev->type == GDK_KEY_PRESS) {
        if (ev->key.keyval == GDK_KEY(Left) || ev->key.keyval == GDK_KEY(KP_Left)) {
            move_pointer(DIR_LEFT);
        } else if (ev->key.keyval == GDK_KEY(Right) || ev->key.keyval == GDK_KEY(KP_Right)) {
            move_pointer(DIR_RIGHT);
        } else if (ev->key.keyval == GDK_KEY(Up) || ev->key.keyval == GDK_KEY(KP_Up)) {
            move_pointer(DIR_UP);
        } else if (ev->key.keyval == GDK_KEY(Down) || ev->key.keyval == GDK_KEY(KP_Down)) {
            move_pointer(DIR_DOWN);
        } else if (ev->key.keyval == GDK_KEY(KP_Home)) {
            move_pointer(DIR_UP_LEFT);
        } else if (ev->key.keyval == GDK_KEY(KP_Page_Up)) {
            move_pointer(DIR_UP_RIGHT);
        } else if (ev->key.keyval == GDK_KEY(KP_End)) {
            move_pointer(DIR_DOWN_LEFT);
        } else if (ev->key.keyval == GDK_KEY(KP_Page_Down)) {
            move_pointer(DIR_DOWN_RIGHT);
        } else if (ev->key.keyval == GDK_KEY(Return) ||
            ev->key.keyval == GDK_KEY(KP_Space) ||
            ev->key.keyval == GDK_KEY(KP_Enter) ||
            ev->key.keyval == GDK_KEY(space)) {
...
void mw_create(gint da_width, gint da_height)
{
    ...
    icon = gdk_pixbuf_new_from_file (PACKAGE_DATADIR "/gtkballs/gtkballs_16x16.png", &error);
    if (icon) {
