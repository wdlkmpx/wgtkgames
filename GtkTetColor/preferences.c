...
{
    GtkWidget *cell_size_spinbutton, *initial_level_spinbutton, *destroy_delay_spinbutton;
    GtkWidget *fontselection;
    gchar *new_font_name = NULL;
    gint temp;

    cell_size_spinbutton = (GtkWidget *) g_object_get_data (G_OBJECT (widget), "cell_size_spinbutton");
    initial_level_spinbutton = (GtkWidget *) g_object_get_data (G_OBJECT (widget), "initial_level_spinbutton");
    destroy_delay_spinbutton = (GtkWidget *) g_object_get_data (G_OBJECT (widget), "destroy_delay_spinbutton");
    fontselection = (GtkWidget *) g_object_get_data (G_OBJECT (widget), "fontselection");

    temp = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (cell_size_spinbutton));
    if (temp != cell_width) {
        cell_width = temp;
    ...
    temp = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (initial_level_spinbutton));
    if (temp != initial_level) {
        initial_level = temp;
        preferences_changed = TRUE;
    }

    temp = gtk_spin_button_get_value_as_int (GTK_SPIN_BUTTON (destroy_delay_spinbutton));
    if (temp != destroy_delay) {
        destroy_delay = temp;
        preferences_changed = TRUE;
    }

    new_font_name = gtk_font_selection_get_font_name (GTK_FONT_SELECTION (fontselection));
    if (new_font_name)
    {
...
void change_font_of_labels (void)
{
    ...
    if (font_name) {
        ...
        if (pango_font_description_get_size (desc)) {
            label = (GtkWidget *) g_object_get_data (G_OBJECT (main_window), label_name[0]);
            ...
            for (i = 0; i < MAX_LABEL; i++) {
                if (i) {
                    label = (GtkWidget *) g_object_get_data (G_OBJECT (main_window), label_name[i]);
                }
